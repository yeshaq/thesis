...
\begin{itemize}
  ...
  \item Very successful model:
    ...
    \item Predictions up to the billionth decimal place
...
\item Massless Neutrinos
\item Does not allow for unification of forces
\item Divergent corrections to the Higgs mass - ``Hierarchy problem''
\end{itemize}
\begin{equation*}
...
%\end{itemize}
\color{red!80}{Any extension of the SM should attempt to solve these shortcomings...}
}
\subsection{SUSY}
\frame{
...
\item Provides additional loop \\corrections to the Higgs mass which cancel the quadratic divergence! 
\end{itemize}
\begin{center}
...
\item $10^{11}$ protons per bunch lead to \\around 20 collisions per \\beam crossing.
\end{itemize}
...
\item pseudo-rapidity: $\eta \equiv −\ln\left( \tan\left(\theta/2\right)\right)$
%\begin{equation*}
...
In this analysis: LSP is assumed to be the Neutralino: $\tilde{\chi}_1^0$ \\
~\\
\begin{columns}[cc]
  \column{0.5\paperwidth}\centering
  ``T2tt''\\
...
\frametitle{Summary of Signal Characteristics}

\color{black}
\begin{itemize}
\setlength{\itemsep}{12pt}
\item Significant hadronic content:
~\\
\begin{center}
...
\item Electroweak backgrounds with genuine \met 
\begin{itemize}
...
control samples with jet systems, containing also isolated $\mu$ or $\gamma$.
...
Control: req's above +
\begin{itemize}
\item photon $\pt > 165~\gev, |\eta| < 1.44$
...
Benefits:
\begin{itemize}
\item reduced dependence on accuracy of MC modeling
\item control and hadronic regions: 
  \begin{itemize}
  \item binned identically 
  \item kinetically similar, 
...
\item between \njet multiplicities: check jes, kinematics 
\end{itemize}
~\\
}
\frame{ 
...
\begin{center}
    $2 \leq \njet \leq 3$\\
  \includegraphics[width=0.8\paperwidth]{../figures/syst/closureTests_pf_take21_le3j.pdf}
\end{center} 
}
...
\begin{center}
    $\njethigh$\\
  \includegraphics[width=0.8\paperwidth]{../figures/syst/closureTests_pf_take21_ge4j.pdf}
\end{center} 
}
...
\begin{table}[!h]
  \caption{Total background systematic uncertainty in \%}
  \centering
  \scriptsize
  \begin{tabular}{ ccccccccc }
    \hline
...
\end{table}
\begin{center}
Numbers above include a 5\% trigger uncertainty 
\end{center}
}
...
\item Bkg predictions are related to $\mu$, $\gamma$ 
...
\begin{itemize}
\item the background yields each hadronic bin
...
%    } 
    \subfigure[Pull versus signal region bin.\label{fig:sigVsCat}]{
      \includegraphics[width=0.6\paperwidth, trim=0 0 0 30, clip=true]{../figures/fit/v22/significances_catVsHt_4}
    } \\
...
\times L_{\mu}^k \times L_{ph}^k \times L_{\rm EWK\, syst.}^k
\end{equation*}
where $k$ is an analysis category.
\begin{equation*}
L_{hadronic}=\prod_i \mathrm{Pois}(n^i |\, b^i + s^i)
\end{equation*}
...
\begin{equation*}
L_{sig.\,syst}=\mathrm{Logn}(1.0 |\,\rho_{sig}, \delta)
\end{equation*}
}
...
%    } 
    \subfigure[T2cc, (2--3,0)]{
      \includegraphics[width=0.48\textwidth,page=6]{../figures/sms/t2cc/v1/T2cc_eff}
    } 
...
%    } 
    \subfigure[T2tt, ($\geq 4$,2)]{
      \includegraphics[width=0.48\textwidth,page=3]{../figures/sms/t2tt/v1/T2tt_eff}
    } \\
...
\begin{figure}[h!]
  \begin{center}
      \includegraphics[width=0.70\textwidth,]{../figures/limits/merged/T2tt/v6/T2tt_mlsp0_xmin300_smooth2_2sigma.pdf}
...
\begin{figure}[h!]
  \begin{center}
      \includegraphics[width=0.70\textwidth,clip=true,]{../figures/limits/merged/T2tt/v6/CLs_frequentist_T2tt_2012pf_1b_ge4j_2b_ge4j_xsLimit.jpg}
     %\caption{\label{fig:upperLimits-t2tt}Expected and observed upper limits on the production cross section 
    %for the models \texttt{T2tt}. Mass pairs left (and below) of the observed limit are excluded at the 95\% C.L.}
  \end{center}
\end{figure}
...
\begin{itemize}
\item fluctuations consistent signal
\end{itemize}
\begin{figure}[h!]
  \begin{center}
...
\begin{figure}[h!]
  ...
  \subfigure[\label{fig:upperLimits-t2cc}Analysis T2cc Limits]{
    \includegraphics[width=0.48\textwidth,clip=true]{../figures/limits/merged/T2cc/v2/CLs_frequentist_T2cc_2012pf_0b_le3j_0b_ge4j_1b_ge4j_xsLimit.jpg}
    }   
  \subfigure[\label{fig:atlas}ATLAS T2cc Limits]{     
    \includegraphics[width=0.48\textwidth,clip=true]{../figures/limits/merged/T2cc/v2/atlas}
    }   
...
\frame{
\frametitle{Comparison with CMS}
\begin{columns}[cc]
\column{0.5\paperwidth}\centering
 Analysis T2cc Limits\\
 \includegraphics[width=0.70\textwidth]{../figures/limits/merged/T2cc/v2/CLs_frequentist_T2cc_2012pf_0b_le3j_0b_ge4j_1b_ge4j_xsLimit.jpg}\\
 Analysis T2tt Limits\\
 \includegraphics[width=0.70\textwidth,clip=true,]{../figures/limits/merged/T2tt/v6/CLs_frequentist_T2tt_2012pf_1b_ge4j_2b_ge4j_xsLimit.jpg}
 \column{0.5\paperwidth}\centering
 CMS Stop Limits\\
 \includegraphics[width=0.9\textwidth,clip=true]{../figures/sms/T2tt_ICHEP2014_All-2.pdf}
\end{columns}
}
...
\begin{itemize}
\setlength{\itemsep}{15pt}
\item Extended AlphaT analysis from 11.5 $\fbinv$ to 19.5 $\fbinv$
\item First use of Particle-Flow with $\alphat$ variable 
\item Concentrated on light stop production models
\begin{itemize}
\setlength{\itemsep}{10pt}
\item T2cc: produced strongest limits for in CMS, $m_{\tilde{t}} < 275~\GeV$ excluded
\item T2tt: limit not as strong, but not as optimized as others 
\end{itemize}
\item Analysis with Particle-Flow will be used and developed in the upcoming $13~\TeV$ LHC run
\end{itemize}
