...
\begin{center}
$\mht$ =$\sum\limits_{j} \vec{\Et}^j$ , $\VEtmiss$,$\met = \sum\limits_{all objects} \Et^j$ 
\end{center}
\item no leptons
...
\item no $\alphat$ cut for muon samples when $\nb \geq 1$\\
...
\item expectations via ``translation factors'' from MC
\end{itemize}
