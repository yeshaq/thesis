...
The background in this search can be seperated into two types: 
...
the remaining events are expected to have little contamination from balanced QCD events.  
However, certain types of rare backgrounds caused by detector innefficiencies
can lead to values of \alphat greater than 0.55. Most of these can be
rejected by dedicated vetoes provided by the CMS MET group as described in 
section~\ref{sec:met-filters}, but two remaining types of rare backgrounds 
still need to be considered.

To protect from accepting events with \mht due to jets with similar values of 
the azymuthal angle ($\phi$) that fail to pass the jet \pt threshold. Such jets would
