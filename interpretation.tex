...
comparing the calculated upper limit with the theoritical cross section shifted up down 
and by its theoritical uncertainty i.e. 
...
intent, figure~\ref{fig:sigVsCat} of chapter~\ref{sec:results} is replotted 
in figure~\ref{fig:sigVsCat2} to isolate the categories of interest in \texttt{T2tt}.
\begin{figure}[h!]
  \begin{center}
...
show the highest pulls: $\sim\!\!2.5\sigma$ and $\sim\!\!1.5\sigma$ respectively.  
In the \njethigh and $\nb = 1$ category, the lowest two \scalht bins show the 
highest pulls with $\sim\!\!1.5\sigma$. The descripency is further investigated by a 
closer study of two bins in  $(m_{\st},m_{\text{LSP}})$.  The first one, $m_{\st} = 400\GeV$ and 
$m_{\text{LSP}} = 0\GeV$ is excluded by the observed data. It is useful to gauge 
the signal signifcance, defined here as the signal yield divided by 
$\sqrt{b+(.1b)^2}$ where $b$ is the SM expectation as determined by a simultaneous 
fit to all under under the SM background hypothesis. Figure~\ref{fig:t2tt-sig-400}
plots the signal signficance in bins of \scalht and shows that most of the signal 
significance is expected in the lowest \scalht bins in both categories. 
...
hypothesis. The signal under consideration is $m_{\st} = 400\GeV$ and 
